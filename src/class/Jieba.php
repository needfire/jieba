...
class Jieba
{
    ...
    public static function init($options = array())
    {
        ...
        } elseif ($options['dict']=='other') {
            if(empty($options['other_dict_name'])){
                exit("缺少字典名称 options['other_dict_name']");
            }
            if(empty($options['other_dict_dir'])){
                exit("缺少字典文件路径 options['other_dict_dir']");
            }
            $f_name = $options['other_dict_name'];
            self::$dictname = $options['other_dict_name'];
        } else {
            exit('请指定字典');
        }
        ...
        if($options['dict'] == 'other'){
            //自定义加载词库，直接传入词库的路径
            self::$trie = Jieba::genTrie($options['other_dict_dir']."/".$f_name);
        }else{
            //加载
